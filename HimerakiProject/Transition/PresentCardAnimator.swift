...
final class PresentCardTransitionDriver {
    ...
    init(params: PresentCardAnimator.Params, transitionContext: UIViewControllerContextTransitioning, baseAnimator: UIViewPropertyAnimator) {
        ...
        let screens: (home: CustomTabController, cardDetail: fullArticleViewController) = (
            ctx.viewController(forKey: .from)! as! CustomTabController,
            ctx.viewController(forKey: .to)! as! fullArticleViewController
        )
        ...
        // Final preparation
        // -------------------------------
        params.fromCell.alpha = 0.0
        params.fromCell.resetTransform()

        let topTemporaryFix = screens.cardDetail.scrollView.topAnchor.constraint(equalTo: cardDetailView.topAnchor, constant: 0)
        ...
        func completeEverything() {
            ...
            cardDetailView.fillSuperview(padding: UIEdgeInsets(top: -1, left: 0, bottom: 0, right: 0))
