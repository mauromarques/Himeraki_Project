...
final class DismissCardAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let screens: (cardDetail: fullArticleViewController, home: CustomTabController) = (
            ctx.viewController(forKey: .from)! as! fullArticleViewController,
            ctx.viewController(forKey: .to)! as! CustomTabController
        )
        ...
        // Fix weird top inset
        let topTemporaryFix = screens.cardDetail.scrollView.topAnchor.constraint(equalTo: cardDetailView.topAnchor)
        topTemporaryFix.isActive = GlobalConstants.isEnabledWeirdTopInsetsFix

        container.layoutIfNeeded()

        // Force card filling bottom
        let stretchCardToFillBottom = screens.cardDetail.scrollView.bottomAnchor.constraint(equalTo: cardDetailView.bottomAnchor)
...
//                screens.cardDetail.isFontStateHighlighted = true
        ...
        UIView.animate(withDuration: transitionDuration(using: ctx), delay: 0.0, options: [.curveEaseInOut], animations: {
            cardDetailView.alpha = 0
        }, completion: nil)
        
        UIView.animate(withDuration: transitionDuration(using: ctx), delay: 0.1, options: [.curveEaseInOut], animations: {
