...
class NewsViewController: StatusBarAnimatableViewController, UICollectionViewDelegateFlowLayout, AddNewDataDelegate {
    ...
    override func viewDidLoad() {
        ...
        refreshControl.addTarget(viewModel, action: #selector(NewsViewModel.refresh), for: .valueChanged)
    ...
    @objc func presentSecretController() {
        ...
        present(secretController, animated: true)
    }
    ...
    func prepareForDatabase(with object: News?) {
        guard let newType = object else {
            showAlert(title: "Ops", message: "An error occurred, please try again", dismissButtonTitle: "Okay")
            return
        }
        viewModel.delete(deletedNew: newType)
        viewModel.addNews(addedNew: newType)
    }
}
