...
var collectionViewItemType = NewsViewModelItemType.article
...
var catArrayFromCSV = [""]
var wordsArrayFromCSV = [""]

func readCsvFromFile(path: String) {
    let stream = InputStream(fileAtPath: path)!
    let csv = try! CSVReader(stream: stream, hasHeaderRow: true)
    let csvHeader = csv.headerRow
    catArrayFromCSV = csvHeader ?? [""]
    for category in csvHeader! {
        let stream2 = InputStream(fileAtPath: path)!
        let csv2 = try! CSVReader(stream: stream2, hasHeaderRow: true)
        wordsArrayFromCSV.removeAll()
        print ("********** \(category) **********")
        while csv2.next() != nil && csv2[category] != "" {
            print ("\(csv2[category] ?? "NOT FOUND")")
            wordsArrayFromCSV.append(csv2[category] ?? "NOT FOUND")
            
        }
        var genericCategory = CategoryData(name: category, prompt: wordsArrayFromCSV)
        categoriesToGetPrompt.append(genericCategory)
    }
}

extension UserDefaults {
    
    var articlesOpened: Int {
